    ...
    d3.json("data/us_congress_2016_lower_48.json").then((json) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < json.features.length; j++) {
          const jsonDistrict =
            json.features[j].properties.STATEFP +
            "-" +
            json.features[j].properties.CD115FP;
      ...
      map
        ...
        .on("mouseover", function (d) {
          ...
          d.properties.candidates.forEach((el) => {
            resultsString = `${resultsString}<p>(${el.party})  ${
              el.candidate
            }: ${d3.format(".1%")(el.percentage)}</p>`;
          });
        ...
        .on("mouseout", function (d) {
          d3.select(this).style("fill", stateFill);
